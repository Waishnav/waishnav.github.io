...
│       └── opus\_iterations
└── tmp\`\`\`​steps
2\) using older experiment critique points, rewrite new frontend design skill for new experiment in `experiments/version-[x]/.agents/skills/frontend-design/SKILLS.md`3) with the freshly written skills open codex instance in `experiments/version-[x]/` with theo's prompt to run actual experiment in fresh context
